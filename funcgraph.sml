...
structure EdgeKey = struct type ord_key = {from:nodeID, to:nodeID, isMoveEdge:bool}
		    fun compare({from=f1,to=t1,isMoveEdge=isMoveEdge1},{from=f2,to=t2,isMoveEdge=isMoveEdge2}) =
...
type 'a edge = {from: nodeID, to: nodeID, isMoveEdge: bool}
...
fun getNodeData (_,a,_,_) = a

fun adjustSets(g,{from,to, isMoveEdge},f) =
...
fun addEdge(g,{from, to, isMoveEdge}) = adjustSets(g,{from=from,to=to,isMoveEdge=isMoveEdge},NodeSet.add)
fun removeEdge(g,{from, to, isMoveEdge}) = adjustSets(g,{from=from,to=to,isMoveEdge=isMoveEdge},NodeSet.delete)
			      handle NotFound => raise NoSuchEdge(from,to)
fun removeEdge'(g,{from, to, isMoveEdge}) = adjustSets(g,{from=from,to=to,isMoveEdge=isMoveEdge},NodeSet.delete)
			       handle NotFound => g


fun doubleEdge (g, temp1, temp2, isMoveEdge) = case Key.compare(temp1,temp2) of
  EQUAL => g
|  _ => let
           val g' = addEdge(g, {from = temp1, to = temp2, isMoveEdge = isMoveEdge})
        in
          addEdge(g', {from = temp2, to = temp1, isMoveEdge = isMoveEdge})
        end


fun removeNode(g,nid) =
    let val (_,_,succ,pred) = getNode(g,nid)
	val es1 = NodeSet.foldl (fn(s,es)=>EdgeSet.add(es,{from=nid,to=s, isMoveEdge = false}))
				EdgeSet.empty
				succ
	val es2 = NodeSet.foldl (fn(p,es)=>EdgeSet.add(es,{from=p,to=nid, isMoveEdge = false}))
				es1
				pred
