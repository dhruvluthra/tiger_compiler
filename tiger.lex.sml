...
fun yyAction42 (strm, lastMatch : yymatch) = (yystrm := strm;
      (stringPos := yypos; YYBEGIN STRING; continue()))
...
fun yyAction47 (strm, lastMatch : yymatch) = let
      ...
      in
        yystrm := strm;
        (ErrorMsg.error yypos ("Error - Illegal character sequence: " ^ yytext); continue())
      end
...
fun yyAction51 (strm, lastMatch : yymatch) = let
      ...
      in
        yystrm := strm;
        (myString := (!myString ^ yytext); YYBEGIN STRING; continue())
      end
fun yyAction52 (strm, lastMatch : yymatch) = (yystrm := strm;
      (myString := (!myString) ^ "\\"; YYBEGIN STRING; continue()))
...
fun yyAction55 (strm, lastMatch : yymatch) = let
      ...
      in
        yystrm := strm;
        (if ((valOf (Int.fromString yytext) > 31) andalso (valOf (Int.fromString yytext) < 256))
...
fun yyAction59 (strm, lastMatch : yymatch) = let
      ...
      in
        yystrm := strm;
        (ErrorMsg.error yypos ("Error - Illegal escape character: " ^ yytext); continue())
      end
...
fun yyQ17 (strm, lastMatch : yymatch) = (case (yygetc(strm))
       of NONE => yyAction59(strm, yyNO_MATCH)
        | SOME(inp, strm') =>
            if inp = #" "
              then yyQ18(strm', yyMATCH(strm, yyAction59, yyNO_MATCH))
            else if inp < #" "
              then if inp = #"\t"
                  then yyQ18(strm', yyMATCH(strm, yyAction59, yyNO_MATCH))
                else if inp < #"\t"
